...
if ((args.size() == 0) || ((args.size() == 1) && (args[0].equalsIgnoreCase("-help") || args[0].equalsIgnoreCase("help")))) {
	...
	println("Starting the SQL Server cli install.");
	if (!readParameters()) 
		return;
    try {
	  ServiceRegistry.getEvaluationService().invokeFunction("system:dbwc_mssql_installer_silentinstaller.runSilentInstaller", [argFglamName, argInstancesFileName, argLockboxName, argLockboxPassword], null, null);
	}  catch(e) {
		while (e.cause != null) {
			e = e.cause;
		}	
		println("The SQL Server cli install failed with error :\n " + e.getMessage() );
	}
	
	println("Finished SQL Server cli install.");
}
